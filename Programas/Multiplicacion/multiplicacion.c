...
int main(int argc, char *argv[]){
    FILE *entrada, *salida;
    char WAVE[4];
    chunkInfo f1Info, f2Info;
    chunkData f1Data, f2Data;
    short bitPer = 0;
    int dataChunk = 0;
    ...
    if(argc < 4)
        ...
        signal1 = fopen(argv[1], "rb");
        signal2 = fopen(argv[2], "rb");
        salida = fopen(argv[3], "wb");
        if(entrada != NULL){
            readFile(entrada1, &f1Info, &f1Data);
            readFile(entrada2, &f2Info, &f2Data);
...
void readFile(FILE *entrada, FILE *salida, char *WAVE, short *bitPer, int *dataChunk){
    for(int i=0; i<4; i++)
        fputc(fgetc(entrada), salida);
    for(int i=0; i<4; i++)
        fgetc(entrada);
...
void fourierTransform(double *data){
